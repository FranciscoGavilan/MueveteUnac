...
import com.example.mueveteunac2.viewUser.ui.viewModel.PointViewModel;
...
public class RouteMapFragment extends Fragment implements OnMapReadyCallback{
    ...
    public void moveMap(Stop stop) {
        if(markerStop!=null){
            markerStop.remove();
        }

        Double stopLatitud,stopLongitud;

        stopLatitud = stop.getStopPosition().getLatitude();
        stopLongitud = stop.getStopPosition().getLongitude();
